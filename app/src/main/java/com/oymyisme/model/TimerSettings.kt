...
data class TimerSettings(
    ...
    fun getFormattedStudyDuration(): String {
        return if (testModeEnabled) {
            "${TestMode.TEST_STUDY_TIME_MS / 1000}秒"
        } else {
            "${studyDurationMin}分钟"
        }
    }
    ...
    fun getFormattedStudyAndBreakDuration(): String {
        return if (testModeEnabled) {
            "${TestMode.TEST_STUDY_TIME_MS / 1000}秒学习+${TestMode.TEST_BREAK_TIME_MS / 1000}秒休息"
        } else {
            "${studyDurationMin}分钟学习+${breakDurationMin}分钟休息"
        }
    }
